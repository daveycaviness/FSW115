...
axios.get("http://api.bryanuniversity.edu/Davey/list/")
    .then(result =>{
        result.data.forEach(function(e){
                var splitTask = e.name.split(" ")
                var taskTop = splitTask.length - 1
                var defineIt = document.createElement("p")
                axios.get(`https://api.dictionaryapi.dev/api/v2/entries/en/${splitTask[taskTop]}`)
                    .then(word =>{
                        defineIt.textContent = `definition: ${word.data[0].meanings[0].definitions[0].definition}`
                    })
...
document.querySelector("#flexContainer").addEventListener("submit", function(e){
    e.preventDefault();
    axios.post("http://api.bryanuniversity.edu/Davey/list/", {
        name: document.querySelector("#title").value, 
        description: document.querySelector("#description").value,
        isComplete: false
    })
        .then(result =>{
            var splitTask = result.data.name.split(" ")
            var taskTop = splitTask.length - 1
            var defineIt = document.createElement("p")
                axios.get(`https://api.dictionaryapi.dev/api/v2/entries/en/${splitTask[taskTop]}`)
                    .then(word =>{
...
//word.data[0].meanings[0].definitions[0]